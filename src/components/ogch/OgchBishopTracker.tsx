...
const BISHOP_CHARACTERS: BishopSeedCharacter[] = [
...
function buildBishopCharacter(
  seed: BishopSeedCharacter,
  ...
  return {
    ...
    nextAvailableAt: overrides?.nextAvailableAt ?? BISHOP_NEXT_AVAILABLE_AT,
...
export default function OgchBishopTracker() {
  const [characters, setCharacters] = useState<OgchCharacterProgress[]>(() =>
    BISHOP_CHARACTERS.map((character) => buildBishopCharacter(character))
  );
  ...
  const openManualEdit = (character: OgchCharacterProgress) => {
  ...
  const updateCharacter = (characterId: string, updater: (current: OgchCharacterProgress) => OgchCharacterProgress) => {
  ...
  const submitManualEdit = (event: FormEvent<HTMLFormElement>) => {
    ...
    updateCharacter(manualTarget.id, (character) =>
      buildBishopCharacter(
        ...
        },
        {
          ...
          cooldownStatus: fromDateTimeLocalInput(manualNextAvailableAt) ? "onCooldown" : "available",
        }
      )
    );
  ...
  const resetCooldown = (character: OgchCharacterProgress) => {
      ...
      <div className="mx-auto max-w-[1440px]">
        <header className="mb-5 flex flex-col gap-3 border-b border-slate-800 pb-5 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-400">Bishop Dungeon Run Tracker</p>
            <p className="mt-1 text-xs text-slate-600">Data: Local bishop roster</p>
          </div>
        ...
        <section className="rounded-xl border border-cyan-500/20 bg-cyan-950/20 px-4 py-3 text-sm font-semibold text-cyan-100">
          Bishop roster uses the same OGCH logic as Windhawk. Current next run target is{" "}
          <span className="font-mono">{formatBangkokDateTime(BISHOP_NEXT_AVAILABLE_AT)}</span>.
        </section>
